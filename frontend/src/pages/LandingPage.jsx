...
const getDefaultImage = (title) => {
  if (!title) return "https://images.unsplash.com/photo-1580522154071-c6ca47a859ad?w=800&q=80";
  const lower = title.toLowerCase();
  if (lower.includes("camera") || lower.includes("lens")) return "https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=800&q=80";
  if (lower.includes("scooter") || lower.includes("bike") || lower.includes("activa")) return "https://images.unsplash.com/photo-1593950315186-76a92975b60c?w=800&q=80";
  if (lower.includes("playstation") || lower.includes("ps5") || lower.includes("console")) return "https://images.unsplash.com/photo-1606813907291-d86efa9b94db?w=800&q=80";
  if (lower.includes("drill") || lower.includes("tools")) return "https://images.unsplash.com/photo-1504148455328-c376907d081c?w=800&q=80";
  if (lower.includes("laptop") || lower.includes("macbook") || lower.includes("pc")) return "https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=800&q=80";
  if (lower.includes("tent") || lower.includes("camp")) return "https://images.unsplash.com/photo-1504280390225-b4618751fa05?w=800&q=80";
  if (lower.includes("drone")) return "https://images.unsplash.com/photo-1507582020474-9a35b7d455d9?w=800&q=80";
  return "https://images.unsplash.com/photo-1580522154071-c6ca47a859ad?w=800&q=80";
};
...
const ProductCard = ({ item, isNight, isBookmarked, onBookmarkToggle, onCardClick, userCoords, coordsLoading, coordsError }) => {
  ...
  const images = item.images && item.images.length > 0 ? item.images : [getDefaultImage(item.title)];
  ...
  return (
    <div className="flex-shrink-0 w-[280px] sm:w-[320px] group relative hover:-translate-y-2 transition-all duration-300 cursor-pointer overflow-hidden rounded-[12px] bg-white border border-slate-200 shadow-sm hover:shadow-md">
      
      {/* Inner Card Container */}
      <div 
        onClick={() => onCardClick?.(item)}
        className="w-full h-full flex flex-col bg-white text-slate-800 relative"
      >
        ...
        <div className="h-44 relative flex items-center justify-center text-6xl group-hover:scale-[1.02] transition-transform duration-300 overflow-hidden bg-[#E2E8F0]">
          {images.length > 0 ? (
            <img src={getImageUrl(images[imgIndex])} alt={item.title} className="w-full h-full object-cover mix-blend-multiply" />
          ) : (
            <span className="select-none">{item.emoji}</span>
          )}

          {/* Carousel controls - Custom Heroicons */}
          <button 
            onClick={handlePrev}
            className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-800 transition-colors z-20 cursor-pointer bg-white/70 hover:bg-white rounded-full p-1 shadow-sm"
          ...
          <button 
            onClick={handleNext}
            className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-800 transition-colors z-20 cursor-pointer bg-white/70 hover:bg-white rounded-full p-1 shadow-sm"
            title="Next Image"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4">
        ...
        <div className="p-4 flex flex-col gap-1.5 flex-grow bg-white">
          <h3 className="font-bold text-[17px] text-slate-800 truncate mb-0.5">
            {item.title}
          </h3>
          
          <div className="flex items-baseline gap-1 mb-1">
            <span className="text-[#183028] font-black text-xl">₹{item.price}</span>
            <span className="text-xs text-slate-600 font-medium">/{item.unit || "day"}</span>
          </div>

          <div className="flex flex-col gap-1.5 mb-3">
            <p className="text-[11px] text-slate-700 font-medium flex items-center gap-1.5">
              Security Deposit: <span className="font-bold">₹{item.securityDeposit || "0"}</span>
            </p>

            <div className="flex items-center justify-between mt-1">
              <p className="text-[11px] text-slate-700 font-medium flex items-center gap-1">
                <span className="text-[#A42530] text-[13px]">📍</span> {item.area || "Local Area"}
              </p>
          ...
          <div className="mt-auto pt-3 border-t border-slate-200 flex justify-between items-center">
            {/* Avatar and Name */}
            {!(item.owner && item.owner.includes("(You)")) && (
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full overflow-hidden bg-[#2C3E50] text-white flex items-center justify-center text-[10px] font-bold">
                   <img src={`https://ui-avatars.com/api/?name=${item.owner || 'User'}&background=random`} alt="avatar" className="w-full h-full object-cover" />
                </div>
            ...
            {/* Buttons depending on owner */}
            {item.owner && item.owner.includes("(You)") ? (
              <div className="w-full flex justify-between items-center gap-2">
                <button 
                  onClick={(e) => { e.stopPropagation(); navigate(`/product/${item.id || item._id}/edit`); }}
                  className="flex-1 border border-[#204138] text-[#204138] bg-transparent hover:bg-slate-50 text-[11px] px-3 py-1.5 rounded-full font-bold transition-colors cursor-pointer text-center"
                >
                  Edit Listing
                </button>
                <button 
                  onClick={(e) => { e.stopPropagation(); navigate(`/product/${item.id || item._id}/manage`); }}
                  className="flex-1 bg-[#204138] text-white hover:bg-[#152e27] text-[11px] px-3 py-1.5 rounded-full font-bold transition-colors cursor-pointer text-center"
                >
                  Manage Availability
                </button>
              </div>
              ...
              <button 
                ...
                className="bg-[#204138] text-white hover:bg-[#152e27] text-[12px] px-5 py-1.5 rounded-full font-bold transition-colors shadow-sm cursor-pointer ml-auto"
              >
                {item.rowType === "Second-Hand" ? "Buy Now" : "Rent Now"}
              </button>
            )}
          </div>
        </div>
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen font-sans overflow-x-hidden bg-[#F4E8D6] text-slate-900 transition-colors duration-500">
      ...
      {/* ── ORIGINAL NAVBAR FROM MOCKUP ── */}
      <nav className="sticky top-0 z-[45] bg-white border-b border-slate-100 shadow-sm transition-colors duration-500">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          
          <div className="flex items-center gap-8">
            {/* Logo */}
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/")}>
              <div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center text-white font-bold text-lg">R</div>
              <span className="text-xl font-bold text-slate-800">RentIt</span>
            </div>
            ...
            <div className="hidden lg:flex items-center gap-1.5 cursor-pointer">
              ...
              <div className="flex flex-col">
                <span className="text-[9px] text-slate-400 font-bold uppercase leading-tight tracking-wider">Services To</span>
                <span className="text-xs font-bold text-slate-800 leading-tight">Add Address</span>
              </div>
            </div>
          </div>

          {/* Center nav links */}
          <div className="hidden md:flex items-center gap-6 text-[13px] font-semibold text-slate-600">
            <button onClick={() => navigate("/rent-catalog")} className="hover:text-indigo-600 transition-colors">Rent Hub</button>
            <button onClick={() => navigate("/second-hand-catalog")} className="hover:text-indigo-600 transition-colors">Second-Hand</button>
            <button onClick={() => navigate("/requests-hub")} className="hover:text-indigo-600 transition-colors">Requests Hub</button>
            <button onClick={() => {
              document.getElementById('how-it-works')?.scrollIntoView({ behavior: 'smooth' });
            }} className="hover:text-indigo-600 transition-colors">How It Works</button>
          </div>

          {/* Right Area: Auth & Post */}
          <div className="flex items-center gap-4">
            <button onClick={() => setIsNight(!isNight)} className="text-yellow-500 text-lg hover:scale-110 transition-transform">
              ☀️
            </button>
            
            <button 
              onClick={() => {
                if (isLoggedIn) setPostModalOpen(true);
                else navigate("/login");
              }} 
              className="bg-indigo-500 text-white text-xs px-4 py-2 rounded-full font-bold hover:bg-indigo-600 transition-colors shadow-sm"
            >
              + Post Item
            </button>
            ...
            {!isLoggedIn ? (
              <div className="flex items-center gap-4">
                <Link to="/login" className="text-xs font-bold text-slate-600 hover:text-indigo-600 transition-colors">
                  Log In
                </Link>
                <Link to="/register" className="bg-indigo-500 text-white text-xs px-4 py-2 rounded-full font-bold hover:bg-indigo-600 transition-colors shadow-sm">
                  Register
                </Link>
              </div>
            ) : (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 p-1 rounded-full border border-slate-200 hover:bg-slate-50 transition-all duration-200 cursor-pointer"
                >
                  {profilePic ? (
                ...
                {profileDropdownOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-48 rounded-2xl shadow-xl border border-slate-100 p-2 z-50 animate-fade-in bg-white text-slate-800">
                      <button
                        onClick={() => { setProfileDropdownOpen(false); navigate("/profile"); }}
                        className="w-full text-left px-4 py-2.5 rounded-xl text-xs font-black transition-colors flex items-center gap-2 hover:bg-slate-50 text-slate-700 cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 rounded-xl text-xs font-black transition-colors flex items-center gap-2 hover:bg-red-50 text-red-650 cursor-pointer"
                      >
                        <span>🚪</span> Logout
                      </button>
                    </div>
      ...
      <section className="relative overflow-hidden py-20 px-4" style={{ background: isNight ? "#09090b" : "#fdf6ee", transition: "background 0.5s ease" }}>
        <img src={landingBg} alt="Background" className="absolute inset-0 w-full h-full object-cover opacity-90" />
        {particles.map((p, i) => (
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
          <div className="slide-left">
            ...
            <h1 className="text-5xl md:text-6xl font-black leading-tight mb-4 text-white drop-shadow-md" style={{ fontFamily: "'Playfair Display', serif" }}>
              Rent Anything.<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400 drop-shadow-sm">Earn Everything.</span>
            </h1>
            <p className="text-lg mb-8 leading-relaxed text-slate-200 drop-shadow">
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              <button onClick={() => navigate("/rent-catalog")} className="flex items-center gap-2 justify-center bg-gradient-to-r from-indigo-500 to-violet-600 text-white px-8 py-4 rounded-2xl font-black text-lg shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-200 active:scale-95 cursor-pointer">
              ...
              <button 
                ...
                className="flex items-center gap-2 justify-center bg-emerald-500 hover:bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black text-lg shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-200 cursor-pointer"
              >
              ...
              <button onClick={() => navigate("/orders")} className="flex items-center gap-2 justify-center bg-white text-indigo-600 px-8 py-4 rounded-2xl font-black text-lg shadow-md hover:shadow-xl border-2 border-indigo-200 hover:border-indigo-400 hover:scale-105 transition-all duration-200 cursor-pointer">
      ...
      <section className="max-w-full px-4 py-16 bg-[#F4E8D6] relative overflow-hidden">
        {/* Geometric background lines matching theme */}
        <svg className="absolute top-0 left-0 w-64 h-64 text-[#EADEC9] opacity-60" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="1"><path d="M-10,50 L50,-10 M-20,60 L60,-20 M-30,70 L70,-30 M-40,80 L80,-40"/></svg>
        <svg className="absolute bottom-0 right-0 w-96 h-96 text-[#EADEC9] opacity-60" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="1"><path d="M50,110 L110,50 M40,120 L120,40 M30,130 L130,30 M20,140 L140,20"/></svg>

        <div className="max-w-7xl mx-auto space-y-16 relative z-10">
        
        {/* Row 1: Items Available For Rent */}
        <div className="space-y-4">
          <div className="flex justify-between items-center border-b border-[#EADEC9] pb-2">
            <h2 className="text-[26px] font-bold tracking-tight text-[#1F3123] flex items-center" style={{ fontFamily: "'Playfair Display', serif" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#C19B5E] mr-3"><circle cx="9" cy="21" r="1"></circle><circle cx="20" cy="21" r="1"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path></svg>
              Items Available For Rent
            </h2>
            <button onClick={() => navigate("/rent-catalog")} className="text-sm font-semibold text-[#204138] hover:text-black flex items-center gap-1 transition-colors">
              View All Rental Hub <span className="text-[16px]">→</span>
            </button>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center border-b border-[#EADEC9] pb-2">
            <h2 className="text-[26px] font-bold tracking-tight text-[#1F3123] flex items-center" style={{ fontFamily: "'Playfair Display', serif" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#C19B5E] mr-3"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>
              Items You Are Listing
            </h2>
            <button onClick={() => navigate("/orders")} className="text-sm font-semibold text-[#204138] hover:text-black transition-colors">
              Manage Orders
            </button>
          </div>
          <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center border-b border-[#EADEC9] pb-2">
            <h2 className="text-[26px] font-bold tracking-tight text-[#1F3123] flex items-center" style={{ fontFamily: "'Playfair Display', serif" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#C19B5E] mr-3"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"></path><line x1="7" y1="7" x2="7.01" y2="7"></line></svg>
              Available For Second-Hand Purchase
            </h2>
            <button onClick={() => navigate("/second-hand-catalog")} className="text-sm font-semibold text-[#204138] hover:text-black transition-colors flex items-center gap-1">
              View Buyout Hub <span className="text-[16px]">→</span>
            </button>
          </div>
          <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-black tracking-tight text-slate-900 flex items-center" style={{ fontFamily: "'Playfair Display', serif" }}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-pink-400 mr-3"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="6"></circle><circle cx="12" cy="12" r="2"></circle></svg>
      ...
      {/* ── HOW IT WORKS ── */}
      <section id="how-it-works" className={`py-20 px-4 transition-colors duration-500 ${isNight ? "bg-slate-900" : "bg-[#F4E8D6]"}`}>
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <span className={`inline-block text-xs font-bold px-4 py-2 rounded-full mb-4 shadow-sm border ${isNight ? "bg-indigo-500/20 text-indigo-300 border-indigo-500/30" : "bg-white text-[#204138] border-[#EADEC9]"}`}>
              🔐 Secure & Simple Process
            </span>
            <h2 className={`text-4xl font-black mb-3 ${isNight ? "text-white" : "text-[#1F3123]"}`} style={{ fontFamily: "'Playfair Display', serif" }}>
              How MyRental101 Works
            </h2>
            <p className={`max-w-xl mx-auto ${isNight ? "text-slate-400" : "text-slate-600"}`}>Our cartoon guides walk you through every step — from verifying your identity to sealing your first deal.</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            <HowCard step="1" cartoon={<CartoonVerify />} title="Verify Your Identity" desc="Upload your Aadhaar/PAN card. Our AI verifies you in 60 seconds — keeping the community safe and trusted." color={isNight ? "border-indigo-400/30 bg-indigo-950/50" : "border-[#EADEC9] bg-white"} isNight={isNight} />
            <HowCard step="2" cartoon={<CartoonPost />} title="Post or Request Items" desc="List your idle items with photos & your price, or post a borrow request with your own price proposal." color={isNight ? "border-green-400/30 bg-green-950/50" : "border-[#EADEC9] bg-white"} isNight={isNight} />
            <HowCard step="3" cartoon={<CartoonNegotiate />} title="Negotiate the Deal" desc="Chat directly with owners or borrowers. Counter-offer, discuss terms, and agree on a fair price in real-time." color={isNight ? "border-amber-400/30 bg-amber-950/50" : "border-[#EADEC9] bg-white"} isNight={isNight} />
            <HowCard step="4" cartoon={<CartoonDeal />} title="Seal & Rent Securely" desc="Money held in escrow. Once both confirm, the deal is done. Rate each other and build your rental reputation!" color={isNight ? "border-violet-400/30 bg-violet-950/50" : "border-[#EADEC9] bg-white"} isNight={isNight} />
          </div>
        </div>
      </section>
