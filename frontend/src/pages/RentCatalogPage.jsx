      ...
      <div className="h-32 w-full flex items-center justify-center text-5xl mb-4 group-hover:scale-105 transition-transform overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-950 relative">
        ...
        {images.length > 1 && (
          <>
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 filter drop-shadow">
...
export default function RentCatalogPage() {
  ...
  const [maxPrice, setMaxPrice] = useState(() => {
    ...
    return val !== null && !isNaN(Number(val)) && Number(val) >= 0 ? Number(val) : 10000;
  });
...
const handleNegotiationClick = async (productId, currentPrice, title) => {
  ...
  useEffect(() => {
    ...
    setMaxPrice(pVal !== null && !isNaN(Number(pVal)) && Number(pVal) >= 0 ? Number(pVal) : 10000);
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      ...
      if (maxPrice !== (currentPrice !== null ? Number(currentPrice) : 10000)) {
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setCoordsError("Distance filter disabled. Allow location access to search nearby.");
          setCoordsLoading(false);
        },
        { timeout: 10000 }
      );
    } else {
      setCoordsError("Distance unavailable. Geolocation not supported by browser.");
      setCoordsLoading(false);
    }
    ...
    if (activeToken) {
      API.get("/rent/products/bookmarks/ids")
        ...
        .catch(err => console.error("Error fetching bookmarks from db in RentCatalogPage:", err));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    let apiUri = `/rent/products?paginated=true&productType=RENT&page=${pageVal}&limit=12`;
    ...
    API.get(apiUri, { signal: controller.signal })
      ...
      .catch(err => {
        ...
        console.error("Error fetching RentCatalogPage products:", err);
        setLoading(false);
      });
  ...
  const handleBookmarkToggle = async (item) => {
    ...
    try {
      ...
      console.error("Error toggling bookmark on server in RentCatalogPage:", err);
      // Fallback
      const existing = JSON.parse(localStorage.getItem("bookmarked_items") || "[]");
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 py-6 px-4 md:px-8 ${isNight ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-800"}`}>
      ...
      <div className="max-w-7xl mx-auto mb-6 flex justify-between items-center flex-wrap gap-4 border-b pb-4 border-slate-100 dark:border-slate-800">
        ...
        <div>
          <h1 className="text-xl md:text-3xl font-black text-right"><ShoppingBag className="w-8 h-8 text-indigo-500 inline mr-2" /> Peer-to-Peer Rental Hub</h1>
